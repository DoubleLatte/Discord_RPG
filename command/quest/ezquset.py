...
async def handle_slime_quest(interaction: Interaction, participants: list[User]):
    ...
    for p in participants:
        ...
        if stats:
            players[user_id] = {"hp": stats["hp"], "atk": stats["atk"], "luk": stats["luk"], "kills": stats["kills"], "gear": len(db.get_inventory(user_id))}
    ...
    for attempt in range(max_attempts):
        ...
        if event_chance < 0.5 and slime is None:
            ...
            await interaction.channel.send(f"<@{lucky_player.id}>가 장비 발견: {gear}! 기회: {remaining_attempts}")
        ...
        if slime:
            ...
            if slime:
                for pid in players:
                    ...
                    if stats["hp"] > 0:
                        stats["hp"] -= slime["atk"]
                        await interaction.channel.send(f"슬라임 반격! <@{pid}>에게 {slime['atk']} 데미지 (HP: {stats['hp']})")
                        if stats["hp"] <= 0:
            ...
            if all(stats["hp"] <= 0 for stats in players.values()):
                ...
                for pid in players:
                    db.update_user_stats(pid, players[pid]["hp"], players[pid]["kills"])
                return

    # 퀘스트 종료 후 DB 업데이트 및 결과 출력
    result_msg = "퀘스트 완료!\n"
    total_kills = sum(stats["kills"] for stats in players.values())
    total_gear = sum(stats["gear"] for stats in players.values())
    if total_kills > 0 or total_gear > 0:
        result_msg += "오늘은 수확이 있다.\n"
        for pid, stats in players.items():
            result_msg += f"<@{pid}> - HP: {stats['hp']}, 슬라임 처치: {stats['kills']}, 장비: {stats['gear']}\n"
            db.update_user_stats(pid, stats["hp"], stats["kills"])
    else:
